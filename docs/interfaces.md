...
1. UI는 메시지와 Runtime Context를 Agent 진입점에 전달하고, 검증된 `StudySpotResult`를 화면에 표시한다. 진입 함수명과 동기·비동기 방식은 아직 정하지 않았다.
...
| 필드 제안 | 타입 | 의미와 검증 |
| --- | --- | --- |
| `region` | `str` 또는 `null` | 희망 지역. 서울 내 지원 여부는 검색 Tool에서 확인한다. |
| `deposit_budget_krw` | `int` 또는 `null` | 보증금 상한, 원 단위, 0 이상. `null`을 0으로 바꾸지 않는다. |
| `monthly_rent_budget_krw` | `int` 또는 `null` | 월세 상한, 원 단위, 0 이상. |
| `target_age` | `str` 또는 `null` | 사용자 목표 연령. “학생”처럼 모호하면 구체화한다. API별 연령 구간 변환은 Tool 담당이 정한다. |
| `operating_hours` | `dict` 또는 `null` | 운영 요일·시작·종료 시간. 예: `day_types`, `start`, `end`, `timezone`. |
| `evaluation_priorities` | `dict` 또는 `null` | 선택 조건. 사용자 선호를 검증된 가중치로 바꾸는 규칙은 합의 필요다. |

금액의 “만원”은 입력 단계에서 원으로 변환한다. 단위가 불명확하면 확인한다. 시간대는 서비스 범위에 맞게 `Asia/Seoul`을 사용하는 안이며, 자정을 넘는 운영시간을 오류로 취급하지 말고 API 시간 구간으로 분해한다. `day_type`, `time_range`의 실제 허용 값은 제공 API 확인 후 고정한다.
...
| `conversation_history` | `list[Message]` | 대화 맥락; 실제 메시지 타입은 프레임워크 선택 후 확정 |
...
원문은 실패 때의 `data` 모양을 정하지 않았다. **연결 제안:** 실패는 `success=false`, `data={}`, 오류 코드·메시지를 채운다. 정상 검색 0건은 `success=true`, `data=[]`로 구분한다. 대체 데이터가 스키마 검증을 통과하면 데이터 확보 성공으로 반환하되 대체 이유와 원본 품질을 별도 메타데이터에 남긴다.

### 출처 메타데이터 보완 제안

원문은 조회 시점·기준 시점·캐시 여부를 관리하도록 요구하지만 `ToolResult`에 해당 필드가 없다. 다음 `metadata` 객체를 추가하는 안을 제안한다. **기존 6개 필드만 허용하는 모델과 호환되는지 합의가 필요하다.**

| `metadata` 필드 제안 | 타입 | 용도 |
| --- | --- | --- |
| `fetched_at` | ISO 8601 `str` | 원천 데이터를 확보한 시점. 캐시 재사용 시 현재 시점으로 덮어쓰지 않음 |
| `data_as_of` | `str` 또는 `null` | 원천 데이터 기준 기간. 월별 자료 등 원래 정밀도를 보존 |
| `is_cached` | `bool` | 캐시 사용 여부. 캐시된 Mock은 `is_cached=true`, `is_mock=true` |
| `missing_fields` | `list[str]` | 일부 누락 항목 |
| `units` | `dict[str, str]` | 금액·수량·비율 등의 단위 |
| `fallback_reason` | `str` 또는 `null` | 실제 조회 실패 후 대체한 사유 |

최종 화면에도 출처·기준 시점·캐시·Mock 여부가 전달되어야 한다. 도입 전에는 최소한 원문의 `evidence`와 `missing_data`에서 이를 설명한다. 자료가 신선해 보이도록 캐시 조회 시각을 원천 데이터 갱신 시각으로 사용하지 않는다.

### 공통 식별자

| 필드 | 형식 제안 | 규칙 |
| --- | --- | --- |
| `commercial_area_id` | `str` | 상권 조인의 기준. 원천 간 매핑을 확인한다. |
...
| --- | --- | --- |
| `search_supported_districts` | `region: str` | 지원 후보 목록, 표시명, 원천 식별 정보, 데이터 제공 범위 |
| `resolve_area_entities` | `area_name: str` | 상권 ID, 행정코드, 좌표; 실패 시 `AREA_NOT_FOUND` |
| `get_academy_demand` | `commercial_area_id: str`, `administrative_code: str`, `target_age: str \| None` | 상권별 학원·교육 수요 원천 지표와 집계 범위 |
| `find_nearby_stations` | `latitude: float`, `longitude: float`, `radius_m: int` | 역 ID, 역명, 위치·거리; 정상 0건은 빈 목록 |
| `get_station_exit_traffic` | `station_id: str`, `day_type: str`, `time_range: str` | 역·출구·요일·시간대별 통행량과 단위 |
| `get_district_congestion` | `commercial_area_id: str`, `date_or_day_type: str`, `time_range: str` | 시간대별 혼잡도·활성도 원천 지표 |
...
| `calculate_market_score` | `academy_data`, `visitor_data`, `station_data`, `activity_data`, `rent_data`, `competition_data` | 후보 식별자, 총점·6개 세부 점수, 누락·신뢰도, 계산 정책 식별 정보 |
| `save_user_preferences` | `user_id: str`, `preferences: dict` | 저장된 허용 선호 항목과 성공·실패 |
...
`area_id`와 `area_ids`는 `commercial_area_id`를 사용하는 별칭으로 통일하는 안이다. 저장 Tool의 `user_id` 인자는 모델이 결정하지 않고 Runtime Context에서 주입·검증한다. `visit_datetime`은 시간대가 포함된 값으로 정규화한다.
...
원문이 명시한 오류 코드는 `AREA_NOT_FOUND`다. 추가 코드로 `UNSUPPORTED_REGION`, `INVALID_ARGUMENT`, `API_TIMEOUT`, `API_ERROR`, `INVALID_TOOL_RESULT`, `INSUFFICIENT_DATA`를 제안한다. 의미와 재시도 여부는 연주·Tool 담당이 확정하며, API 오류 원문·키·수신처를 그대로 사용자에게 노출하지 않는다.

## 5. 최종 응답

### StudySpotResult 원문 필드

| 필드 | 타입 | 제약 |
| --- | --- | --- |
| `status` | `Literal["success", "need_more_information", "no_result", "approval_required"]` | 네 가지 값만 허용 |
| `recommendations` | `list[MarketRecommendation]` | 필수, 최대 3개 |
| `requires_approval` | `bool` | 필수, 보고서 전송·일정 등록 승인 대기 시 `true` |

### MarketRecommendation 원문 필드
...
| `evidence` | `list[str]` | 수치의 근거와 실제 출처 |
...
### 추가 질문과 승인 UI를 위한 확장 제안

원문 모델만으로는 `need_more_information`에서 실제 질문을, `approval_required`에서 승인할 내용을 구조적으로 전달하기 어렵다. 다음 필드를 추가하거나 별도 UI 응답 envelope로 감싸는 두 방법 중 하나를 팀이 선택해야 한다.

| 위치 | 필드 제안 | 용도 |
| --- | --- | --- |
| 최상위 응답 | `message: str` | 사용자에게 보여줄 설명 |
| 최상위 응답 | `questions: list[str]` | 필요한 추가 질문 |
| 최상위 응답 | `missing_conditions: list[str]` | 아직 없는 입력 필드명 |
| 최상위 응답 | `reason_code: str \| None` | 미지원, 정책 차단, 반복 한도, 실행 실패 등 사유 |
| 최상위 응답 | `pending_action: dict \| None` | 대기 행동 ID, 종류, 안전한 미리보기, 승인 대상 버전 |
| 최상위 응답 | `action_result: dict \| None` | 저장·전송·일정의 성공, 실패, 거절, 성공 여부 불명 결과 |
| 추천 항목 | `commercial_area_id: str` | 관심 상권 선택·저장과 Tool 근거 연결 |
| 추천 항목 | `data_provenance: list[dict]` | 사용한 Tool별 출처·기준 시점·캐시·Mock 메타데이터 |

**상태별 불변 조건 제안:** 분석 성공은 추천 1~3개를 반환하고, 추가 정보 요청·분석 실패에서는 새 추천을 반환하지 않는다. 승인 대기는 `status="approval_required"`, `requires_approval=true`, 유효한 `pending_action`을 함께 요구한다. 다른 상태는 `requires_approval=false`다. 행동 성공·거절에는 빈 추천 목록과 `action_result`를 허용하고, 실패·확인 필요는 `no_result`와 세부 행동 결과로 설명한다.

확장 필드를 채택하기 전까지 UI와 Agent가 서로 다른 형식을 임의로 구현하지 않는다. Pydantic 모델은 효주가 관리하고 석휘·명하·연주가 상태별 필수값과 검증을 함께 맞춘다.
...
| 누락 | 필수 지표가 무엇인지, 일부 지표만 있을 때 계산 허용 여부, 가중치 재분배 여부 |
...
합의 전에는 누락 지표를 0으로 채우거나 신뢰도를 임의로 0.8 같은 값으로 정하지 않는다. 계산 Tool은 정책상 산출 불가 상태를 반환할 수 있어야 하며 Agent는 이를 `no_result` 또는 후보 제외로 연결한다.
