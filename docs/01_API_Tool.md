# 역할 1 — SK 학원·지하철 API Tool 설계 및 테스트

담당자: **중우** · 브랜치: `feature/api-tools` · 선행 문서: [공통 계약](00_공통계약.md)

## 목표

상권·행정구역·역 식별자를 받아 학원 수요와 지하철 데이터를 공통 형식으로 제공한다. 다음 지시를 기준으로 직접 개발하며, 점수 계산과 Agent 제어는 다른 담당자에게 맡긴다.
...
| `data/mock/academy_*.json`, `data/mock/subway_*.json` | 정상·빈 결과·부분 누락 fixture. 내부 반환 스키마를 실데이터와 동일하게 유지 |
| `tests/test_academy_tools.py`, `tests/test_subway_tools.py` | 각 Tool 단위·계약 테스트 |
...
상세 인자는 공통 계약의 추가 제안이며 역할 4·5와 skeleton 단계에서 확정한다.
...
역할 5의 지역 식별자 → 본 역할의 원천 데이터 → 역할 4의 점수 Tool → 역할 5의 추천 조립 순서다.

재시도·캐시·Fallback 정책의 소유자는 역할 3이다. 본 역할은 **외부 요청 1회**와 timeout·오류 분류, Mock 로더·캐시 어댑터 연결점을 제공한다. Middleware가 ToolResult 실패를 감지해 정책을 적용하도록 협의한다. API 내부에서도 2회 재시도하여 총 호출이 중첩되지 않게 한다.
...
3개 Tool이 같은 계약을 사용하고 정상·실패·누락·Mock 경로가 자체 테스트를 통과한다. 점수 담당자가 API 구현을 몰라도 fixture로 점수 개발을 진행할 수 있어야 한다. 공식 사양 확인표와 실제 연결 검증 여부를 인계한다.
...
- [ ] 공통 계약 PR 체크리스트 충족
- [ ] 3개 Tool 이름·타입 힌트·호출 목적 docstring 제공
